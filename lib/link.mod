...
-- asynchronous handshaked (parallel) 
-- communication link using acknowledged dual rail
-- data lines with completion detection 
-- though still using synchronous logic.
--
--
--    $ENDOFINFO
--
...
begin
  ...
  --
  interface(#rhs:logic[2*$datawidth],#lhs:logic,
            #lhs:logic[2*$datawidth],#rhs:logic);
  --
  -- External monitor signal attached to link
  --
  select(#lhs:logic);
end;
...
begin
  ...
  signal LINK_$O_DIN: std_logic_vector[$datawidth];
  signal LINK_$O_DIN2: std_logic_vector[2*$datawidth];
  signal LINK_$O_DIN_SAMPLED: std_logic_vector[$datawidth*2];
  signal LINK_$O_DOUT_REG: std_logic_vector[$datawidth];
  signal LINK_$O_DOUT2: std_logic_vector[2*$datawidth];
...
begin
  variable samples1 : std_logic_vector[$datawidth*2];
  variable samples2 : std_logic_vector[$datawidth*2];
  if $CLK then
  begin
    if $RES then
    begin
      samples1 := to_logic(0,$datawidth*2);
      samples2 := to_logic(0,$datawidth*2);
      LINK_$O_DIN_SAMPLED <= to_logic(0,$datawidth*2);
    end
    else
    begin
      samples2 := samples1;
...
begin
  variable valid: logic;
  variable empty: logic;
  valid := '1';
  empty := '1';
  for i = 0 to $datawidth-1 do
  begin
    valid := valid and (LINK_$O_DIN_SAMPLED[2*i] xor LINK_$O_DIN_SAMPLED[2*i+1]);
    empty := empty and (not LINK_$O_DIN_SAMPLED[2*i] and not LINK_$O_DIN_SAMPLED[2*i+1]);
    LINK_$O_DIN[i] <= LINK_$O_DIN_SAMPLED[2*i];
  end;
  LINK_$O_DIN_COMPL <= valid;
  LINK_$O_DIN_EMPTY <= empty;
end;
...
begin
  ...
  begin
    LINK_$O_DOUT2 <= to_logic(0,$datawidth*2);
  end
  else
  begin
    for i = 0 to $datawidth-1 do
    begin
      LINK_$O_DOUT2[2*i] <= LINK_$O_DOUT_REG[i];
      LINK_$O_DOUT2[2*i+1] <= not LINK_$O_DOUT_REG[i];
    end;
  end;
end;
