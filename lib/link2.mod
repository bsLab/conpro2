...
begin
  ...
  signal LINK_$O_DIN_SAMPLED: std_logic_vector[2];
  ...
  begin
    ...
    begin
      ...
      begin
        ...
        --
        if LINK_$O_DIN_CNT = to_logic($datawidth,$cntwidth) then
      ...
      begin
        ...
        begin
          ...
          begin
            ...
            if LINK_$O_DOUT_CNT = to_logic($datawidth-1,$cntwidth) then 
            begin
              LINK_$O_DOUT_CNT <= to_logic(0,$cntwidth);
            end;
          end;
...
begin
  ...
  begin
    if $RES then
    begin
      samples1 := to_logic(0,2);
      samples2 := to_logic(0,2);
      LINK_$O_DIN_SAMPLED <= to_logic(0,2);
    end
    else
    begin
      samples2 := samples1;
...
begin
  LINK_$O_DIN_COMPL <= LINK_$O_DIN_SAMPLED[0] xor LINK_$O_DIN_SAMPLED[1];
  LINK_$O_DIN_EMPTY <= not LINK_$O_DIN_SAMPLED[0] and not LINK_$O_DIN_SAMPLED[1];
  LINK_$O_DIN <= LINK_$O_DIN_SAMPLED[0];
end;
