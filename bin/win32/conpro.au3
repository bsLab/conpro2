...
; ConPro GUI Native WIN32 Version
; Set flag
$SelectCount = 0
...
If  FileExists($InstallationPath&'\vumrun.exe') Then
  $DrivePrefix = 'C:'
  $Program1 = $InstallationPath&'\vumrun.exe'
EndIf
If  FileExists($InstallationPath&'\SciTE.exe') Then
  $Editor = $InstallationPath&'\SciTE.exe'
EndIf

If  FileExists('C:'&'/opt/Conpro2/bin/win32/vumrun.exe') Then
  $DrivePrefix = 'C:'
...
While 1
  ...
  Switch $idMsg[0]
    ...
    Case $idCompile
      ...
      $_pid = Run($Program1 &' -w '& $Program2 & ' -notty ' & $sCompileOptions & ' -O "' & $DrivePrefix&$sSourceDir & '" -I "' & $DrivePrefix&$sSourceDir & '" ' & $sSourceFile, @SystemDir, @SW_HIDE, $STDERR_CHILD + $STDOUT_CHILD)
      $answer = ''
    ...
    Case $idProj
      If $sSourceFile <> "" Then
        ...
        _GUICtrlEdit_AppendText($idLog,$Program2 & @CRLF)
        _GUICtrlEdit_AppendText($idLog,$sSourceFile & @CRLF)
        $_pid = Run($Program1 &' -w '& $Program2 & ' +report -notty -O "' & $DrivePrefix&$sSourceDir & '" -I "' & $DrivePrefix&$sSourceDir & '" -parse ' & $sSourceFile, @SystemDir, @SW_HIDE, $STDERR_CHILD + $STDOUT_CHILD)
        $answer = ""
