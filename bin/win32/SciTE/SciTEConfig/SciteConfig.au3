...
#Region ;**** Directives created by AutoIt3Wrapper_GUI ****
	...
	#AutoIt3Wrapper_Run_After=for %I in ("%in%" "%out%" "AbbrevMan.au3" "Scite_Reload_Props.au3" "StringSize.au3" "Get_AU3_RegistrySettings.au3" "UCTMan.au3" ) do copy %I "..\..\Programs_Updates\SciTEConfig"
	#AutoIt3Wrapper_Run_After_Admin=y
	#AutoIt3Wrapper_Run_After=for %I in ("%in%" "%out%" "AbbrevMan.au3" "Scite_Reload_Props.au3" "StringSize.au3" "Get_AU3_RegistrySettings.au3" "UCTMan.au3" ) do copy %I "C:\Program Files (x86)\autoit3\SciTE\SciTEConfig"
...
#Region ; GUI
	...
	Global $FontSize = "10"
	Opt("GUICoordMode", 1)
	Global $hSciTEConfig_GUI = GUICreate("SciTE Config for AutoIt3 ver:" & $VERSION, 410, 525, Default, Default, Default, $WS_EX_TOPMOST)
	Global $NeedsSave = 0
	Global $SaveNeeded = 0
...
;~ Global $H_CheckUpdatesSciTE4AutoIt3
	...
	Global $h_tab = GUICtrlCreateTab(10, 10, 390, 460)
...
;~ $H_CheckUpdatesSciTE4AutoIt3 = GUICtrlCreateCheckbox("Check daily for Updates of SciTE4AutoIT3.", $BaseX + 10, $BaseY + 50)
	...
	$BaseY = 40
	; Calltip colour
	GUICtrlCreateGroup("Inline Errors", $BaseX, $BaseY, 340, 100)
	$H_InLineError_Display = GUICtrlCreateCheckbox("Display inline errors in script.", $BaseX + 10, $BaseY + 20)
	Global $H_InLineWarning_Label = GUICtrlCreateLabel("Inline Warning Color", $BaseX + 5, $BaseY + 50, 150, 20, 0x0200)
	Global $H_InLineError_Label = GUICtrlCreateLabel("Inline Error Color", $BaseX + 5, $BaseY + 70, 150, 20, 0x0200)
	Global $H_InLineWarning_Color = GUICtrlCreateButton("Fore", $BaseX + 170, $BaseY + 50, 80, 20)
	Global $H_InLineWarning_BkColor = GUICtrlCreateButton("Back", $BaseX + 170 + 80, $BaseY + 50, 80, 20)
	Global $H_InLineError_Color = GUICtrlCreateButton("Fore", $BaseX + 170, $BaseY + 70, 80, 20)
	Global $H_InLineError_BkColor = GUICtrlCreateButton("Back", $BaseX + 170 + 80, $BaseY + 70, 80, 20)
	;
	$BaseY = 145
	;
	...
	Global $H_CaretLine_Label = GUICtrlCreateLabel("Caret Line Color", $BaseX + 5, $BaseY, 150, 20, 0x0200)
	...
	Global $H_CaretLine_Color = GUICtrlCreateButton("Background", $BaseX + 170, $BaseY, 190, 20)
	...
	Global $Status_Msg = GUICtrlCreateLabel("", 10, 475, 390, 17, $SS_SUNKEN)
	;GUICtrlSetBkColor(-1, 0xC0C0C0)
	Global $H_Update = GUICtrlCreateButton("Save+Apply", 10, 495, 70)
	GUICtrlSetState(-1, $GUI_DISABLE)
	GUICtrlSetTip(-1, "Show changes in SciTE and save to SciTEUser.properties")
	Global $H_NewScheme = GUICtrlCreateButton("Change Scheme", 90, 495, 100)
	GUICtrlSetTip(-1, "Select a new Color&Font Scheme.")
	GUICtrlSetState($H_NewScheme, $GUI_HIDE)
	Global $H_Exit = GUICtrlCreateButton("Exit", 200, 495, 50) ; #######################################
...
;~ 			MsgBox(262144 + 48, "SciTE config", "Updated Configuration")
			...
			Case $Msg = $H_InLineError_Color
				...
				GUICtrlSetColor($H_InLineWarning_Label, $InLineError_Color)
				$NeedsSave = 1
				;
...
#Region ; Funcs
	...
	Func _AlphaFactor($Colour, $iAlpha)
		Local $iFactor = .8 * (255 - $iAlpha) / 255 ; The .8 is a fudge to make the colour look right - probably a monitor thing
		Local $aColours[3]
...
;~ 	$CheckuUpdatesSciTE4AutoIt3 = Number(SendSciTE_GetInfo($My_Hwnd, $SciTE_hwnd, "askproperty:check.updates.scite4autoit3"))
		...
		$InLineWarning_Color = "0X" & StringRegExpReplace($InLineWarning_Prop, "fore:#(.*),back.*", "$1")
		$InLineWarning_BkColor = "0X" & StringRegExpReplace($InLineWarning_Prop, ".*back:#(.*)", "$1")
		Local $InLineError_Prop = SendSciTE_GetInfo($My_Hwnd, $SciTE_hwnd, "askproperty:style.error.2")
		$InLineError_Color = "0X" & StringRegExpReplace($InLineError_Prop, "fore:#(.*),back.*", "$1")
		$InLineError_BkColor = "0X" & StringRegExpReplace($InLineError_Prop, ".*back:#(.*)", "$1")
		...
		$CallTip_Color = "0X" & StringRegExpReplace($CallTip_Prop, "fore:#(.*),back.*", "$1")
		$CallTip_BkColor = "0X" & StringRegExpReplace($CallTip_Prop, ".*back:#(.*)", "$1")
	...
	;
	Func Reset_Status()
		GUICtrlSetBkColor($Status_Msg, 0xF9FEB8)
...
;~ 	EndIf
		...
		If $CallTip_Above Then
			SendSciTE_Command(0, $SciTE_hwnd, "extender:dostring editor.CallTipPosition(1)")
		Else
			SendSciTE_Command(0, $SciTE_hwnd, "extender:dostring editor.CallTipPosition(0)")
		EndIf
...
;~ 				ConsoleWrite("! Key not Found: " & $aKeyValue & @CRLF)
	...
	Func Update_status($NewText, $iColour = 0x00FF00)
		...
		GUICtrlSetBkColor($Status_Msg, $iColour)
