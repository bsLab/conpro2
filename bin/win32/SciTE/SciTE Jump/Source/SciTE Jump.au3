...
#cs
	...
	Last Textual Changes In This Script:		14th April 2014
...
#pragma compile(FileVersion, 2.18.103.243)
...
_SetVariables()
Global Const $__sScriptDir = _GetInstalledSettingsDir() ; If AutoIt & SciTE is installed then use the application directory instead.
...
Global Const $SCITEJUMP_SCRIPTS = 'au3;bat;cmd;html;htm;log;txt'
...
Global Enum $__fGUIIsMinimised, _
...
$__vGUIAPI[$__fGUIIsMinimised] = False

Global Enum $__fMiscFindInFile, $__fMiscFunctionSnippet, $__fMiscIsSearchUDFs, $__fMiscIsSort, _
		$__fMiscSearchFocus, $__fMiscSearchParams, _
		$__fMiscToggleFoldOrNoRefresh, _
...
$__vMiscAPI[$__fMiscFindInFile] = False
$__vMiscAPI[$__fMiscFunctionSnippet] = False
$__vMiscAPI[$__fMiscIsSearchUDFs] = _IsEx('SearchUDFs', 0) = 1
$__vMiscAPI[$__fMiscIsSort] = _IsEx('Sort', 0) = 1
$__vMiscAPI[$__fMiscSearchFocus] = _IsEx('SearchFocus', 0) = 1
$__vMiscAPI[$__fMiscSearchParams] = BitOR($__vMiscAPI[$__fMiscSearchParams], _IsEx('SearchInComments', 0) * $__iTreeViewIsComment)
$__vMiscAPI[$__fMiscSearchParams] = BitOR($__vMiscAPI[$__fMiscSearchParams], _IsEx('SearchInFunctions', 1) * ($__iTreeViewIsFunctionOrFile + $__iTreeViewIsCustomFunction))
$__vMiscAPI[$__fMiscSearchParams] = BitOR($__vMiscAPI[$__fMiscSearchParams], _IsEx('SearchInRegions', 0) * $__iTreeViewIsRegion)
$__vMiscAPI[$__fMiscToggleFoldOrNoRefresh] = False

Global Enum $__iMiscLastFunctionFuncName, $__iMiscLastFunctionTreeViewID, $__fMiscLastFunctionSelect
...
Func _Main()
	Local $aGUI_Window = False, _
			...
			$fDockSelection = False, _
			$iCount = 0, $iMsg = -9999, _
			$sReturn = '', _
	...
	Else
		...
		$aStateDockWindows[1] = '+'
	EndIf
	GUICtrlSetData($__vGUIAPI[$__iGUIDockWindow], $aStateDockWindows[$fDockSelection])
	...
	GUICtrlSendMsg($__vGUIAPI[$__iGUIInputFind], $EM_SETCUEBANNER, False, _GetLanguage('TIP_SEARCH_5', 'Search in Scripts') & ' (' & $SCITEJUMP_SCRIPTS & ')')
	...
	_Toggle_CheckOrUnCheck($iSearchUDFs, $__vMiscAPI[$__fMiscIsSearchUDFs])
	...
	_Toggle_CheckOrUnCheck($iSortFunctions, $__vMiscAPI[$__fMiscIsSort])
	...
	_Toggle_CheckOrUnCheck($iSearchInputFocus, $__vMiscAPI[$__fMiscSearchFocus])
	...
	_Toggle_CheckOrUnCheck($iSearchInComments, BitAND($__vMiscAPI[$__fMiscSearchParams], $__iTreeViewIsComment) = $__iTreeViewIsComment)
	...
	_Toggle_CheckOrUnCheck($iSearchInFuctions, BitAND($__vMiscAPI[$__fMiscSearchParams], ($__iTreeViewIsFunctionOrFile + $__iTreeViewIsCustomFunction)) = ($__iTreeViewIsFunctionOrFile + $__iTreeViewIsCustomFunction))
	...
	_Toggle_CheckOrUnCheck($iSearchInRegions, BitAND($__vMiscAPI[$__fMiscSearchParams], $__iTreeViewIsRegion) = $__iTreeViewIsRegion)
	...
	While 1
		...
		Switch $iMsg
			...
			Case $GUI_EVENT_MINIMIZE
				$__vGUIAPI[$__fGUIIsMinimised] = True
				_WinAPI_SetFocus(GUICtrlGetHandle($__vGUIAPI[$__iGUIProgressBar]))
			...
			Case $aTabs[$eTabID] ; Hide The TreeView.
				Switch GUICtrlRead($aTabs[$eTabID], 1)
					...
					Case $aTabs[$eTabTwo]
						...
						GUICtrlSetPos($__vGUIAPI[$__iGUITreeViewCurrent], -9999, Default, Default, Default)
			...
			Case $__vGUIAPI[$__iGUIDockWindow]
				GUICtrlSetData($__vGUIAPI[$__iGUIDockWindow], $aStateDockWindows[Not $fDockSelection])

				If $fDockSelection Then ; Undock SciTE Jump.
					_Monitor_Check(False, 'Monitor_Pause')
				...
				EndIf
				$fDockSelection = Not $fDockSelection
				$vDummyValue = 0
			...
			Case $iEnter
				Switch _WinAPI_GetDlgCtrlID(_WinAPI_GetFocus())
					Case $__vGUIAPI[$__iGUIInputFind]
						...
						$__vMiscAPI[$__fMiscFindInFile] = True
			...
			Case $__vGUIAPI[$__iGUIRefresh]
				If $__vMiscAPI[$__fMiscToggleFoldOrNoRefresh] Then ; Don't refresh if the Cancel button is selected.
					$__vMiscAPI[$__fMiscToggleFoldOrNoRefresh] = False
				Else
					...
					Else
						...
						$__vTreeViewParentsAPI[$__iTreeViewParentsTreeViewStart] = -9999
						$__vTreeViewParentsAPI[$__iTreeViewParentsTreeViewEnd] = -9999
			...
			Case $iSearchInputFocus
				$__vMiscAPI[$__fMiscSearchFocus] = _IsChecked($iSearchInputFocus)

				_IsWrite('SearchFocus', $__vMiscAPI[$__fMiscSearchFocus])
			...
			Case $iSearchInComments, $iSearchInFuctions, $iSearchInRegions
				...
				$__vMiscAPI[$__fMiscSearchParams] = 0
			...
			Case $iSearchUDFs
				$__vMiscAPI[$__fMiscIsSearchUDFs] = _IsChecked($iSearchUDFs)

				_IsWrite('SearchUDFs', $__vMiscAPI[$__fMiscIsSearchUDFs])
				ControlClick($__vSciTEAPI[$__hSciTEResponseGUI], '', $__vGUIAPI[$__iGUIRefresh])
			...
			Case $iSortFunctions
				$__vMiscAPI[$__fMiscIsSort] = _IsChecked($iSortFunctions)

				_IsWrite('Sort', $__vMiscAPI[$__fMiscIsSort])
				ControlClick($__vSciTEAPI[$__hSciTEResponseGUI], '', $__vGUIAPI[$__iGUIRefresh])
			...
			Case $iUDF_Add
				If FileCopy($__vCurrentFileAPI[$__sCurrentFilePath], $sIncludePath, Int(_IsChecked($iUDF_Overwrite) And _IsEnabled($iUDF_Overwrite))) Then
					_Flash($__vSciTEAPI[$__hSciTEResponseGUI], $COLOR_LIME)
			...
			Case $iWM_COPYDATA ; If the WM_COPYDATA message is interecepted then parse the command that was passed.
				...
				Switch $vDummyValue
					Case 'GUI_Activate'
						If $__vGUIAPI[$__fGUIIsMinimised] Then
			...
			Case $__vTreeViewParentsAPI[$__iTreeViewParentsSearchStart] To $__vTreeViewParentsAPI[$__iTreeViewParentsSearchEnd]
				...
				If $iMsg Then
					If $__vMiscAPI[$__fMiscFunctionSnippet] Then
						$__vMiscAPI[$__fMiscFunctionSnippet] = False
						...
						$vDummyValue = 0
					Else
						If $__vMiscAPI[$__fMiscFindInFile] Then
						...
						Else
							...
							Else
								...
								If $__vMiscAPI[$__fMiscToggleFoldOrNoRefresh] Then
	...
	Local Const $fRestart = $vDummyValue == 'Monitor_Exit_Restart'
	$sReturn = ''
	$vDummyValue = ''
...
Func _AddToSciTEJump($fWrite = True)
	...
	Local $fIsSciTEJump = StringRegExp($sData, '(?im)^command\.name\.\d+\.\$\(au3\)=' & $sProgramName) = 1, _
			$iIndex = -1
	If Not $fIsSciTEJump And $fWrite Then
		Local $aReturn = StringRegExp('command.0.iCount' & @CRLF & $sData, '(?im)^command\.(?:name\.)?(\d+)\.', 3), _
				$fSwapped = False, $iValue = 0
	...
	If $fWrite And $iIndex > -1 Then
		$fIsSciTEJump = _SetFile('# ' & $iIndex & ' guinness ' & $sProgramName & @CRLF & _
...
Func _ArrayListView($hWnd, ByRef Const $aArray, $fIsCheckBoxes, $sHeader = '', $sTitle = 'Array Display', $sBeforeString = '', $sAfterString = '', $iCaseSensitive = 1, $iRegularExpression = 0)
	...
	While 1
		If $__vMiscAPI[$__iMiscAddHeaderOrOpenLocation] >= 0 Then
			_ReplaceInFileCompare(_GUICtrlListView_GetItemText($__vGUIAPI[$__iGUIArrayListView], $__vMiscAPI[$__iMiscAddHeaderOrOpenLocation]), -9999, $sBeforeString, $sAfterString, $iCaseSensitive, $iRegularExpression)
...
Func _CreateJumpArray(ByRef $aAllAnchors, ByRef $sData)
	...
	If $__vMiscAPI[$__fMiscIsSearchUDFs] Then
...
Func _Export_FunctionRegionList($sFilePath, $sFileName, $iFileSize, $sFileFolder, $iControlID = -9999)
...
Func _Find($sFindText, $sDirectory)
	...
	_LanguageAdd($__aLanguageItems, 7, $__vTreeViewParentsAPI[$__iTreeViewParentsFind], 'TIP_SEARCH_1', 'Search results')
	...
	If $sFindText = '' Then
		...
		Next
	Else
		_ReplaceInFiles($__vGUIAPI[$__iGUIRefresh], $aReturn, $aSearch, $sFindText, $sFindText, False, $SCITEJUMP_SCRIPTS, 0, 0)
		If @error Then
			$aReturn = 0
			Return SetError(1, 0, False)
		EndIf
	...
	If $aReturn[0][0] = 0 Then
		...
		_LanguageAdd($__aLanguageItems, 8, $__vTreeViewParentsAPI[$__iTreeViewParentsFindStart], 'TIP_SEARCH_2', 'No search results')
		Return SetError(2, 0, False)
	EndIf
	_LanguageAdd($__aLanguageItems, 8, '', '', '')
...
Func _FindDestroy($fReset = Default)
	_Toggle_ShowOrHide($__vGUIAPI[$__iGUITreeViewFind], False)
	...
	$__vMiscAPI[$__fMiscFindInFile] = False
	If $fReset Or $fReset = Default Then
		$__aFindItems[0][0] = 0
		$__vTreeViewParentsAPI[$__iTreeViewParentsFindStart] = -9999
		$__vTreeViewParentsAPI[$__iTreeViewParentsFindEnd] = -9999
	EndIf
...
Func _GetTitle()
	Local Const $aOSArch[2] = ['', ' (64-bit)'], $aPortable[2] = [' (Portable)', '']
...
Func _GUICtrlTreeView_ContextMenu($hGUI, $iIndex, $iSubItem, $iMode = 1) ; 0 = Find In Files ContextMenu, 1 = TreeView Items ContextMenu, 2 = Search ContextMenu, 3 = TreeView Items (Basic) ContextMenu, 4 = Array Display
	...
	Local Const $aExport[$iExportUbound + 1][3] = [['HTML', 'HTML', 1],['LATEX', 'LATEX', 2],['PDF', 'PDF', 3],['RTF', 'RTF', 4],['XML', 'XML', 5],['', '', ''],['FUNCTION_LIST', 'Function and Region list', 6]], _
			$fIsAutoIt = _IsValidFileType($__vCurrentFileAPI[$__sCurrentFilePath], 'au3') = 1
	...
	If Not ($iIndex = -1) And Not ($iSubItem = -1) Then ; Won't Show These MenuItem(s) Unless An Item Is Selected.
		...
		If $iMode > 0 And $iMode < 4 Then
			...
			If $fIsAutoIt And Not ($iMode = 3) Then
	...
	Switch $iControlID
		...
		Case $iContextItem4
			$__vMiscAPI[$__fMiscFunctionSnippet] = True
			$iControlID = 4
			$iError = 0

		Case $iContextItem5
			$__vMiscAPI[$__fMiscToggleFoldOrNoRefresh] = True
			$iControlID = 5
			$iError = 0
...
Func _INIUnicode($sINI)
	...
	Else
		...
		If Not ($iEncoding = $FO_UNICODE) Then
			Local Const $sData = _GetFile($sINI, $iEncoding)
			If @error Then
				$fReturn = False
			EndIf
...
Func _IsInstalled()
	...
	If Not $fReturn Then $fReturn = FileExists(EnvGet('SciTE_USERHOME') & '\abbrev.properties') Or FileExists(EnvGet('SciTE_HOME') & '\abbrev.properties')
	Return $fReturn
...
Func _IsUpdate($sFilePath, $fOverride = Default)
...
Func _IsX64()
	Local Const $iOSArch = _OSArch(), $sProgramName = _ProgramName()
	Local $sCommandLine = $CmdLineRaw
	If @OSArch = 'X64' And @AutoItX64 = 0 And FileExists(@ScriptDir & '\' & $sProgramName & $iOSArch & '.exe') = 1 Then
		If Not (StringStripWS($sCommandLine, $STR_STRIPALL) == '') Then
...
Func _Monitor()
	If $__vGUIAPI[$__fGUIIsMinimised] Or $__vMiscAPI[$__fMiscFindInFile] Then
		Return False
	EndIf
	Local Const $fIsActive = _IsActive($__vSciTEAPI[$__hSciTEWindow])
...
Func _Monitor_Check($fClose, $sSendString = '')
	Local $hWnd = 0
	...
	If $aDockList[0][0] Then
		If $fClose Or $aDockList[0][0] > 1 Then
...
Func _Monitor_Run($fClose = Default)
	Local Const $hWnd = _Monitor_Check($fClose)
	If $hWnd = 0 Then
...
Func _ProgressMarquee_Start($iControlID = -1)
	...
	Return GUICtrlSendMsg($iControlID, $PBM_SETMARQUEE, 1, 50) = True
EndFunc   ;==>_ProgressMarquee_Start

Func _ProgressMarquee_Stop($iControlID = -1, $fReset = Default, $fHide = Default)
	Local Const $fReturn = GUICtrlSendMsg($iControlID, $PBM_SETMARQUEE, 0, 50) = True
	If $fReset Then
...
Func _ReplaceInFiles($iControlID, ByRef $aReturn, $aSearch, $sBeforeString, $sAfterString, $fReplace = Default, $sFileExtensions = 'txt', _
	...
	Local Const $sCmdLine = '\MonitorRename "<filepath>' & $sFilePath & '</filepath>' & _
			...
			'<replacestring>' & Int($fReplace) & '</replacestring>' & _
...
Func _Search($sSearchText, ByRef $aArray)
	...
	_LanguageAdd($__aLanguageItems, 5, $__vTreeViewParentsAPI[$__iTreeViewParentsSearch], 'TIP_SEARCH_1', 'Search results')
	Local $sData = ''
	For $i = 1 To $aArray[0][0]
		$sData = $aArray[$i][$__iTreeViewSearchBefore]
		If StringInStr($sData, $sSearchText, $STR_NOCASESENSE) And _
				BitAND($__vMiscAPI[$__fMiscSearchParams], $aArray[$i][$__iTreeViewItemID]) = $aArray[$i][$__iTreeViewItemID] Then
	...
	If $__aSearchItems[0][0] = 0 Then
		...
		_LanguageAdd($__aLanguageItems, 6, $__vTreeViewParentsAPI[$__iTreeViewParentsSearchStart], 'TIP_SEARCH_2', 'No search results')
	Else
		$__vTreeViewParentsAPI[$__sTreeViewParentsSearch] &= '&'
...
Func _SearchDestroy($fReset = Default)
	_Toggle_ShowOrHide($__vGUIAPI[$__iGUITreeViewSearch], False)
	...
	If $fReset Or $fReset = Default Then
		...
		$__vTreeViewParentsAPI[$__iTreeViewParentsSearchStart] = -9999
		$__vTreeViewParentsAPI[$__iTreeViewParentsSearchEnd] = -9999
	EndIf
...
Func _Toggle_EnableOrDisable($iControlID, $fOverride = Default)
	Local Const $aState[2] = [$GUI_ENABLE, $GUI_DISABLE]
	If $fOverride = Default Then
		$fOverride = Int(BitAND(GUICtrlGetState($iControlID), $aState[0]) = $aState[0])
	Else
		$fOverride = Int(Not $fOverride)
	EndIf
	GUICtrlSetState($iControlID, $aState[$fOverride])
EndFunc   ;==>_Toggle_EnableOrDisable

Func _Toggle_ProgressBar($fEnable = Default)
	If $fEnable Then
...
Func _Toggle_ShowOrHide($iControlID, $fOverride = Default)
	Local Const $aState[2] = [$GUI_SHOW, $GUI_HIDE]
	If $fOverride = Default Then
		$fOverride = Int(BitAND(GUICtrlGetState($iControlID), $aState[0]) = $aState[0])
	Else
		If $iControlID < 1 Then
...
Func _TreeViewCreate($fSetCurrent = Default)
...
Func _TreeView_Refresh($fEmpty = False, $sData = 'TIP_ERROR_1', $sDefault = 'Empty file.')
	...
	$__vTreeViewParentsAPI[$__iTreeViewParentsFunction] = -9999
	If $fEmpty Then
		...
		_LanguageAdd($__aLanguageItems, 1, $__vTreeViewParentsAPI[$__iTreeViewParentsComment], $sData, $sDefault)
		Return False
	EndIf
	...
	If $vAllAnchorsAndClear[0][0] = 0 Then
		...
		_LanguageAdd($__aLanguageItems, 1, $__vTreeViewParentsAPI[$__iTreeViewParentsComment], 'TIP_ERROR_2', 'No jump data found.')
		Return SetError(1, 0, False)
	EndIf

	Local Const $fCancelButton = $vAllAnchorsAndClear[0][0] > 7500 ; Display the cancel button if more than 7500 items are in the script file.
	If $fCancelButton Then
		_Toggle_ProgressBar(True)
	EndIf
	...
	$__vTreeViewParentsAPI[$__iTreeViewParentsComment] = -9999
	If BitAND($vAllAnchorsAndClear[0][2], $__iTreeViewIsComment) Then
		...
		_LanguageAdd($__aLanguageItems, 1, $__vTreeViewParentsAPI[$__iTreeViewParentsComment], 'COMMENTS', 'User comments')
	EndIf
	...
	If BitAND($vAllAnchorsAndClear[0][2], $__iTreeViewIsFunctionOrFile) Then
		...
		_LanguageAdd($__aLanguageItems, 2, $__vTreeViewParentsAPI[$__iTreeViewParentsFunction], 'FUNCTIONS', 'Functions')
	EndIf

	$__vTreeViewParentsAPI[$__iTreeViewParentsRegion] = -9999
	If BitAND($vAllAnchorsAndClear[0][2], $__iTreeViewIsRegion) Then
		...
		_LanguageAdd($__aLanguageItems, 3, $__vTreeViewParentsAPI[$__iTreeViewParentsRegion], 'REGIONS', 'Regions')
	EndIf

	$__vTreeViewParentsAPI[$__iTreeViewParentsUDF] = -9999
	If BitAND($vAllAnchorsAndClear[0][2], $__iTreeViewIsCustomFunction) Then
		...
		_LanguageAdd($__aLanguageItems, 4, $__vTreeViewParentsAPI[$__iTreeViewParentsUDF], 'UDFS', 'User Defined')
	EndIf
	...
	$aLastFunction[$__fMiscLastFunctionSelect] = Int($aLastFunction[$__fMiscLastFunctionSelect]) = 1
	...
	If $__vMiscAPI[$__iMiscExpandState] = $__iTreeViewParentsFunction And $aLastFunction[$__iMiscLastFunctionTreeViewID] Then
		...
		If $aLastFunction[$__fMiscLastFunctionSelect] Then
...
Func _Update($fOverride = Default)
	...
	If Not $fOverride And FileExists($sSciTEJump) = 0 Then
...
Func _WinWait($sTitle, $iTime)
	...
	Local $fWinExists = False

	While 1
		$fWinExists = WinExists($sTitle) = 1
		If $fWinExists Or TimerDiff($hTimer) > $iTime Then
			ExitLoop
		EndIf
		Sleep(10)
	WEnd
		...
		Case $__vGUIAPI[$__iGUIInputFind]
			Switch $iHiWord
				...
				Case $EN_SETFOCUS
					$__vMiscAPI[$__sMiscSearchedText] = ''
					$vInput = GUICtrlRead($__vGUIAPI[$__iGUIInputFind])
					If $vInput And $__vMiscAPI[$__fMiscFindInFile] Then
						_TreeViewCreate(False)
						_SearchCreate(False)
						_FindCreate()
					Else
			...
			Switch $iHiWord
				...
				Case $EN_MAXTEXT
					...
					Else
						...
						_SearchCreate(False)
						_SearchDestroy()
						If $__vMiscAPI[$__fMiscFindInFile] And $__aFindItems[0][0] > 0 Then
							_TreeViewCreate(False)
							_FindCreate()
						Else
							_FindDestroy()
			...
			Switch $iIDFrom
				...
				Case $__vGUIAPI[$__iGUITreeView], $__vGUIAPI[$__iGUITreeViewFind], $__vGUIAPI[$__iGUITreeViewSearch]
					...
					If BitAND(DllStructGetData($tHitTest, 'Flags'), $TVHT_ONITEM) Then
						...
						If $__vMiscAPI[$__fMiscFindInFile] Then
							$iMode = 0
						ElseIf $__vMiscAPI[$__fMiscFindInFile] = False And $iIDFrom = $__vGUIAPI[$__iGUITreeView] And Not _IsEmpty($__vCurrentFileAPI[$__sCurrentFilePath]) Then
							...
							Else
								$iMode = 1
							EndIf
						ElseIf $__vMiscAPI[$__fMiscFindInFile] = False And $iIDFrom = $__vGUIAPI[$__iGUITreeViewSearch] And Not _IsEmpty($__vCurrentFileAPI[$__sCurrentFilePath]) Then
							$iMode = 2
						EndIf

						Local $fIsChild = 0
						For $i = 1 To $__vTreeViewParentsAPI[$__iTreeViewParentsCount]
							$fIsChild += Int(_GUICtrlTreeView_IsParent($__vGUIAPI[$__iGUITreeViewCurrent], $__vTreeViewParentsAPI[$i], $iItem))
						Next
						$fIsChild = $fIsChild > 0

						If $iMode >= 0 And $fIsChild Then
