...
Func _Monitor_Rename($sData)
	...
	Local Const $fReplace = Int($aArray[$eReplaceString]) = 1
	Local Const $iCaseSensitive = Int($aArray[$eCaseSensitive])
	Local Const $fRegularExpression = Int($aArray[$eRegularExpression]) = 1
...
Func _Monitor_Run()
	...
	If @error = 0 Then
		...
		Local $fIsDockRight = _GetDockState($sScriptDir & '\Settings.ini'), _
				$fIsFunctionToLine = _Is('MonitorLine', 0, Default, $sScriptDir) = 1, _
				$hTimer = 0, _
				$iPos_1 = 0, $iPos_2 = 0, _
				$sAu3Script = _SciTE_GetCurrentFile(), $sFunction = '', _
				$iAu3Size = FileGetSize($sAu3Script), _
				$vDummyValue = 0
		If $fIsFunctionToLine Then
		...
		While 1
			If $fIsContinue Then
				If Not _DockToWindow($hSciTEGUI, $hResponseGUI, $iPos_1, $iPos_2, $fIsDockRight, 0) Then
					ExitLoop
				EndIf
			EndIf
				...
				Case $iWM_COPYDATA ; If the WM_COPYDATA message is interecepted then parse the command that was passed.
					Switch _SciTE_ParseReturnString(_WM_COPYDATA_GetData(), 2)
						...
						Case 'Monitor_SettingsChange'
							_SciTE_Startup()
							$fIsDockRight = _GetDockState($sScriptDir & '\Settings.ini')
							$fIsFunctionToLine = _Is('MonitorLine', 0, Default, $sScriptDir) = 1
							$sAu3Script = _SciTE_GetCurrentFile()
							If $fIsFunctionToLine Then
								$__vFunctionLines = _FunctionToLine($sAu3Script)
							EndIf
