#
# Tool Interface Tool leonardo.par
# Version 2.05
#
TOP="par"
OBJDIR="obj"
...
VHDL[1]="par"
VHDL[2]="par_FUN_parity1"
VHDL[3]="par_FUN_parity2"
VHDL[4]="par_FUN_parity3"
VHDL[5]="par_FUN_parity4"
VHDL[6]="par_FUN_parity5"
VHDL[7]="par_FUN_parity6"
VHDL[8]="par_main"
do_xpat()
{
  echo [Starting xpat...]
  CMD="xpat -l obj/par_simu.pat"
  EXEC=$CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd par par par_simu"
  echo [$CMD"]" >> "par.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "par.log"
  echo $_TIME >> "par.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" par.vhd"
  echo [$CMD"]" >> "par.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "par.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file par.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_par.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_par.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD par.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"par\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d_rd\",\":2\",\"X\",OUT,\"63 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"p_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="p_rd"
  fi
...
if [ $# = 0 ]
then
  echo "Usage: par.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
