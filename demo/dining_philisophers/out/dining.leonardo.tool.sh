#
# Tool Interface Tool leonardo.dining
# Version 2.06
#
TOP="dining"
OBJDIR="obj"
...
VHDL[1]="dining"
VHDL[2]="dining_init"
VHDL[3]="dining_philosopher_0"
VHDL[4]="dining_philosopher_1"
VHDL[5]="dining_philosopher_2"
VHDL[6]="dining_philosopher_3"
VHDL[7]="dining_philosopher_4"
VHDL[8]="dining_main"
VHDL[9]="conpro"
do_xpat()
{
  echo [Starting xpat...]
  CMD="xpat -l obj/dining_simu.pat"
  EXEC=$CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  echo "dining C" >> "CATAL_ASIMUT_VASY"
  VSTFILES["1"]=""
  VSTFILES=( `ls "*.vst"` )
  ...
  CMD=$BIN"/asimut -bdd dining dining dining_simu"
  echo [$CMD"]" >> "dining.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "dining.log"
  echo $_TIME >> "dining.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" dining.vhd"
  echo [$CMD"]" >> "dining.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "dining.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file dining.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_dining.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_dining.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD dining.vhd" >> $TCL
...
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    DATE=$_DATE
    echo $DATE >> "dining.log"
    echo $DATE
    echo [$SCRAM" "$SRC"]" >> "dining.log"
    echo [$SCRAM" "$SRC"]"
    _TIME=`gdate +%s`
    EXEC=$SCRAM" "$SRC
    { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "dining.log"
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"dining\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"stat_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="stat_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"thinking_0_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="thinking_0_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"thinking_1_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="thinking_1_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"thinking_2_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="thinking_2_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"thinking_3_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="thinking_3_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"thinking_4_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="thinking_4_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"eating_0_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="eating_0_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"eating_1_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="eating_1_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"eating_2_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="eating_2_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"eating_3_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="eating_3_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"eating_4_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="eating_4_rd"
  fi
  IND=`expr $IND + "1"`
...
init()
{
  ...
  cd "obj"
  rm -f "dining.log"; touch "dining.log"
}
check()
{
  if [ ! $res = "0" ]
...
if [ $# = 0 ]
then
  echo "Usage: dining.leonardo.tool.sh [init vhdl synth simu xpat patc pat vst asimut]"
fi
for TARGET in $@ 
do
