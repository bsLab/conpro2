#
# Tool Interface Tool leonardo.e
# Version 2.05
#
TOP="e"
OBJDIR="obj"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" e.vhd"
  echo [$CMD"]" >> "e.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "e.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file e.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_e.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_e.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD e.vhd" >> $TCL
