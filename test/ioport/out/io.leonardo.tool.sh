#
# Tool Interface Tool leonardo.io
# Version 2.05
#
TOP="io"
OBJDIR="obj"
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd io io io_simu"
  echo [$CMD"]" >> "io.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "io.log"
  echo $_TIME >> "io.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" io.vhd"
  echo [$CMD"]" >> "io.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "io.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file io.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_io.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_io.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD io.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"io\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"top_xleds\",\":2\",\"X\",INOUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "INOUT" = "IN" ]
  then
    PORTINP[$IND]="top_xleds"
  fi
  IND=`expr $IND + "1"`
...
if [ $# = 0 ]
then
  echo "Usage: io.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
