#
# Tool Interface Tool leonardo.t
# Version 2.06
#
TOP="t"
OBJDIR="obj"
...
do_patc()
{
  ...
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"x\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="x"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"y\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="y"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"rl\",\":2\",\"X\",IN,\"9 downto 0\",\"\");" >> $PATC
  if [ "IN" = "IN" ]
  then
    PORTINP[$IND]="rl"
  fi
...
if [ $# = 0 ]
then
  echo "Usage: t.leonardo.tool.sh [init vhdl synth simu xpat patc pat vst asimut]"
fi
for TARGET in $@ 
do
