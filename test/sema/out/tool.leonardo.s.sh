...
do_scram()
{
  ...
  do
    ...
    echo $DATE >> "s.log"
    echo $DATE
    echo "["$SCRAM" "$SRC"]" >> "s.log"
    echo "["$SCRAM" "$SRC"]"
    _TIME=`gdate +%s`
    EXEC=$SCRAM" "$SRC
    $EXEC 2>&1 | tee -a "s.log"
    echo $_TIME >> "s.log"
    _EXEC_RES=$?
    res=$_EXEC_RES
    rm -rf "work._savant_lib"
...
do_synth()
{
  ...
  echo "set output_file s.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_s.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_s.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD s.vhd" >> $TCL
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" ""s"".vhd"
  echo "["$CMD"]" >> "s.log"
  EXEC=$CMD
  $EXEC 2>&1 | tee -a "s.log"
  _EXEC_RES=$?
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd ""s"" ""s"" ""s""_simu"
  echo "["$CMD"]" >> "s.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  $EXEC 2>&1 | tee -a "s.log"
...
if [ $# = 0 ]
then
  echo "Usage: tool.leonardo.s.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
