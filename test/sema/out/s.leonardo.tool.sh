...
{
  echo "[Starting xpat...]"
  do_xpat_CMD="xpat -l obj/s_simu.pat"
  EXEC=$do_xpat_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  echo "s C" >> "CATAL_ASIMUT_VASY"
  do_asimut_VSTFILES=( `ls *.vst` )
  for do_asimut_file in ${do_asimut_VSTFILES[@]}
  do
    echo $do_asimut_file" C" >> "CATAL_ASIMUT_VASY"
  done
  do_asimut_CMD=$BIN"/asimut -bdd ""s"" ""s"" ""s""_simu"
  echo "["$do_asimut_CMD"]" >> "s.log"
  _TIME=`gdate +%s`
  EXEC=$do_asimut_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "s.log"
  echo $_TIME >> "s.log"
...
do_pat()
{
  ...
  do_pat_CMD=$BIN"/genpat ""s""_pat"
  echo "["$do_pat_CMD"]" >> "s.log"
  EXEC=$do_pat_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "s.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_vst()
{
  ...
  do_vst_CMD=$VHD2" -nomod "$SXLIB_COMP" ""s"".vhd"
  echo "["$do_vst_CMD"]" >> "s.log"
  EXEC=$do_vst_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "s.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_synth()
{
  ...
  echo "set output_file s.vhd" >> $do_synth_TCL
  ...
  echo "pd" >> $do_synth_TCL
  echo "optimize .work.MOD_s.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $do_synth_TCL
  echo "optimize_timing .work.MOD_s.main" >> $do_synth_TCL
  ...
  echo "auto_write -format VHD s.vhd" >> $do_synth_TCL
  echo "[Forking Leonardo Spectrum...]"
  do_synth_CMD=$SPECTRUM" -f "$do_synth_TCL
  echo "["$do_synth_CMD"]" >> "s.log"
  EXEC=$do_synth_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
  check
  cat "leospec.log" >> "s.log"
}
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    do_scram_DATE=$_DATE
    echo $do_scram_DATE >> "s.log"
    echo $do_scram_DATE
    echo "["$SCRAM" "$do_scram_SRC"]" >> "s.log"
...
do_patc()
{
  ...
  echo "  DEF_GENPAT(\"s\");" >> $do_patc_PATC
  do_patc_IND=1
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"x_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="x_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"y_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="y_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"z_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="z_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"d_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="d_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
...
if [ $# = 0 ]
then
  echo "Usage: s.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
