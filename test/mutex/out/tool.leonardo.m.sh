...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"m\");" >> $PATC
  IND=1
  IND=`expr $IND + 1`
  echo "  DECLAR(\"x_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="x_rd"
  fi
  IND=`expr $IND + 1`
  echo "  DECLAR(\"y_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="y_rd"
  fi
  IND=`expr $IND + 1`
  echo "  DECLAR(\"z_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="z_rd"
  fi
...
do_scram()
{
  ...
  do
    ...
    echo $DATE >> "m.log"
    echo $DATE
    echo "["$SCRAM" "$SRC"]" >> "m.log"
    echo "["$SCRAM" "$SRC"]"
    _TIME=`gdate +%s`
    EXEC=$SCRAM" "$SRC
    $EXEC 2>&1 | tee -a "m.log"
    echo $_TIME >> "m.log"
    _EXEC_RES=$?
    res=$_EXEC_RES
    rm -rf "work._savant_lib"
...
do_synth()
{
  ...
  echo "set output_file m.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_m.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_m.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD m.vhd" >> $TCL
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" ""m"".vhd"
  echo "["$CMD"]" >> "m.log"
  EXEC=$CMD
  $EXEC 2>&1 | tee -a "m.log"
  _EXEC_RES=$?
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd ""m"" ""m"" ""m""_simu"
  echo "["$CMD"]" >> "m.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  $EXEC 2>&1 | tee -a "m.log"
...
if [ $# = 0 ]
then
  echo "Usage: tool.leonardo.m.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
