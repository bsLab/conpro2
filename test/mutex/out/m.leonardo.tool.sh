...
{
  echo "[Starting xpat...]"
  do_xpat_CMD="xpat -l obj/m_simu.pat"
  EXEC=$do_xpat_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  echo "m C" >> "CATAL_ASIMUT_VASY"
  do_asimut_VSTFILES=( `ls *.vst` )
  for do_asimut_file in ${do_asimut_VSTFILES[@]}
  do
    echo $do_asimut_file" C" >> "CATAL_ASIMUT_VASY"
  done
  do_asimut_CMD=$BIN"/asimut -bdd ""m"" ""m"" ""m""_simu"
  echo "["$do_asimut_CMD"]" >> "m.log"
  _TIME=`gdate +%s`
  EXEC=$do_asimut_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "m.log"
  echo $_TIME >> "m.log"
...
do_pat()
{
  ...
  do_pat_CMD=$BIN"/genpat ""m""_pat"
  echo "["$do_pat_CMD"]" >> "m.log"
  EXEC=$do_pat_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "m.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_vst()
{
  ...
  do_vst_CMD=$VHD2" -nomod "$SXLIB_COMP" ""m"".vhd"
  echo "["$do_vst_CMD"]" >> "m.log"
  EXEC=$do_vst_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "m.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_synth()
{
  ...
  echo "set output_file m.vhd" >> $do_synth_TCL
  ...
  echo "pd" >> $do_synth_TCL
  echo "optimize .work.MOD_m.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $do_synth_TCL
  echo "optimize_timing .work.MOD_m.main" >> $do_synth_TCL
  ...
  echo "auto_write -format VHD m.vhd" >> $do_synth_TCL
  echo "[Forking Leonardo Spectrum...]"
  do_synth_CMD=$SPECTRUM" -f "$do_synth_TCL
  echo "["$do_synth_CMD"]" >> "m.log"
  EXEC=$do_synth_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
  check
  cat "leospec.log" >> "m.log"
}
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    do_scram_DATE=$_DATE
    echo $do_scram_DATE >> "m.log"
    echo $do_scram_DATE
    echo "["$SCRAM" "$do_scram_SRC"]" >> "m.log"
...
do_patc()
{
  ...
  echo "  DEF_GENPAT(\"m\");" >> $do_patc_PATC
  do_patc_IND=1
...
if [ $# = 0 ]
then
  echo "Usage: m.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
