...
do_fpga()
{
  ...
  rm -f  "p_xc3s1000-ft256-5.xsvf" "p_xc3s1000-ft256-5.svf" "p_xc3s1000-ft256-5.bit"
  CMD="impact.cmd"
  rm -f $CMD; touch $CMD
  echo "setMode -bsfile" >> $CMD
  echo "setCable -port svf -file p_xc3s1000-ft256-5.svf" >> $CMD
  ...
  check
  rm -f $CMD; touch $CMD
  echo "svf2xsvf -fpga -r 0 -rlen 1024 -i p_xc3s1000-ft256-5.svf -o p_xc3s1000-ft256-5.xsvf" >> $CMD
  echo "quit" >> $CMD
  EXEC="impact -batch "$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "p.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
  res=$_EXEC_RES
  cp "p_xc3s1000-ft256-5.xsvf" "../."
}
do_prom()
{
  echo [creating xsvf prom file...]
  rm -f  "p_xc18v04.xsvf" "p_xc18v04.svf"
  CMD="impact.cmd"
  rm -f $CMD; touch $CMD
  ...
  rm -f $CMD; touch $CMD
  echo "setMode -bsfile" >> $CMD
  echo "setCable -port svf -file p_xc18v04.svf" >> $CMD
  ...
  check
  rm -f $CMD; touch $CMD
  echo "svf2xsvf  -i p_xc18v04.svf -o p_xc18v04.xsvf" >> $CMD
  echo "quit" >> $CMD
  EXEC="impact -batch "$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "p.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_ucf()
{
  ...
  then
    ...
    rm -f $UCF; touch $UCF
    IND="1"
    echo "#NET \"top_p_leds<0>\" LOC = \"\";
    #NET \"top_p_leds<1>\" LOC = \"\";
    #NET \"top_p_leds<2>\" LOC = \"\";
    #NET \"top_p_leds<3>\" LOC = \"\";
    #NET \"top_p_leds<4>\" LOC = \"\";
    #NET \"top_p_leds<5>\" LOC = \"\";
    #NET \"top_p_leds<6>\" LOC = \"\";
    #NET \"top_p_leds<7>\" LOC = \"\";" >> $UCF
...
{
  echo [starting map...]
  EXEC="map -p xc3s1000-ft256-5 "$MAPSET" -o p_map.ncd p.ngd p.pcf"
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "p.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
  res=$_EXEC_RES
...
do_trans()
{
  ...
  EXEC="ngdbuild -p xc3s1000-ft256-5 -uc ../p.ucf -dd . p.ngc p.ngd"
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "p.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
  res=$_EXEC_RES
...
make_xst_runscript()
{
  ...
  CMD="run -ifn p.prj -ifmt mixed -ofn p -ofmt NGC -p xc3s1000-ft256-5 "$SYNSET" -top MOD_p"
  echo $CMD >> "p.xst_run"
}
finish()
{
