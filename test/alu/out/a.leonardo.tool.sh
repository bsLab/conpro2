#
# Tool Interface Tool leonardo.a
# Version 2.05
#
TOP="a"
OBJDIR="obj"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" a.vhd"
  echo [$CMD"]" >> "a.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "a.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file a.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_a.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_a.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD a.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"a\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d_rd\",\":2\",\"X\",OUT,\"15 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d_rd"
  fi
...
if [ $# = 0 ]
then
  echo "Usage: a.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
