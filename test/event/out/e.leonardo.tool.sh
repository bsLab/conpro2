...
{
  echo "[Starting xpat...]"
  do_xpat_CMD="xpat -l obj/e_simu.pat"
  EXEC=$do_xpat_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  echo "e C" >> "CATAL_ASIMUT_VASY"
  do_asimut_VSTFILES=( `ls *.vst` )
  for do_asimut_file in ${do_asimut_VSTFILES[@]}
  do
    echo $do_asimut_file" C" >> "CATAL_ASIMUT_VASY"
  done
  do_asimut_CMD=$BIN"/asimut -bdd ""e"" ""e"" ""e""_simu"
  echo "["$do_asimut_CMD"]" >> "e.log"
  _TIME=`gdate +%s`
  EXEC=$do_asimut_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "e.log"
  echo $_TIME >> "e.log"
...
do_pat()
{
  ...
  do_pat_CMD=$BIN"/genpat ""e""_pat"
  echo "["$do_pat_CMD"]" >> "e.log"
  EXEC=$do_pat_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "e.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_vst()
{
  ...
  do_vst_CMD=$VHD2" -nomod "$SXLIB_COMP" ""e"".vhd"
  echo "["$do_vst_CMD"]" >> "e.log"
  EXEC=$do_vst_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "e.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_synth()
{
  ...
  echo "set output_file e.vhd" >> $do_synth_TCL
  ...
  echo "pd" >> $do_synth_TCL
  echo "optimize .work.MOD_e.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $do_synth_TCL
  echo "optimize_timing .work.MOD_e.main" >> $do_synth_TCL
  ...
  echo "auto_write -format VHD e.vhd" >> $do_synth_TCL
  echo "[Forking Leonardo Spectrum...]"
  do_synth_CMD=$SPECTRUM" -f "$do_synth_TCL
  echo "["$do_synth_CMD"]" >> "e.log"
  EXEC=$do_synth_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
  check
  cat "leospec.log" >> "e.log"
}
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    do_scram_DATE=$_DATE
    echo $do_scram_DATE >> "e.log"
    echo $do_scram_DATE
    echo "["$SCRAM" "$do_scram_SRC"]" >> "e.log"
...
do_patc()
{
  ...
  echo "  DEF_GENPAT(\"e\");" >> $do_patc_PATC
  do_patc_IND=1
...
if [ $# = 0 ]
then
  echo "Usage: e.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
