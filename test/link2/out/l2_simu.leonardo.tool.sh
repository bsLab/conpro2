#
# Tool Interface Tool leonardo.l2_simu
# Version 2.06
#
TOP="l2_simu"
OBJDIR="obj"
...
VHDL[1]="l2_simu"
VHDL[2]="/home/sbosse/proj/conpro2/test/link2/out/l_connect"
VHDL[3]="/home/sbosse/proj/conpro2/test/link2/out/l2"
VHDL[4]="/home/sbosse/proj/conpro2/test/link2/out/l2_p1"
VHDL[5]="/home/sbosse/proj/conpro2/test/link2/out/l2_main"
VHDL[6]="conpro"
do_xpat()
{
  echo [Starting xpat...]
  CMD="xpat -l obj/l2_simu_simu.pat"
  EXEC=$CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  echo "l2_simu C" >> "CATAL_ASIMUT_VASY"
  VSTFILES["1"]=""
  VSTFILES=( `ls "*.vst"` )
  ...
  CMD=$BIN"/asimut -bdd l2_simu l2_simu l2_simu_simu"
  echo [$CMD"]" >> "l2_simu.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l2_simu.log"
  echo $_TIME >> "l2_simu.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" l2_simu.vhd"
  echo [$CMD"]" >> "l2_simu.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l2_simu.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file l2_simu.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_l2_simu.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_l2_simu.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD l2_simu.vhd" >> $TCL
...
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    DATE=$_DATE
    echo $DATE >> "l2_simu.log"
    echo $DATE
    echo [$SCRAM" "$SRC"]" >> "l2_simu.log"
    echo [$SCRAM" "$SRC"]"
    _TIME=`gdate +%s`
    EXEC=$SCRAM" "$SRC
    { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l2_simu.log"
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"l2_simu\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc2_x_rd\",\":2\",\"X\",OUT,\"11 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
  ...
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc1_x_rd\",\":2\",\"X\",OUT,\"11 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
...
init()
{
  ...
  cd "obj"
  rm -f "l2_simu.log"; touch "l2_simu.log"
}
check()
{
  if [ ! $res = "0" ]
...
if [ $# = 0 ]
then
  echo "Usage: l2_simu.leonardo.tool.sh [init vhdl synth simu xpat patc pat vst asimut]"
fi
for TARGET in $@ 
do
