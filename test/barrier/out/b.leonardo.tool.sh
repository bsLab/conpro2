...
{
  echo "[Starting xpat...]"
  do_xpat_CMD="xpat -l obj/b_simu.pat"
  EXEC=$do_xpat_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  echo "b C" >> "CATAL_ASIMUT_VASY"
  do_asimut_VSTFILES=( `ls *.vst` )
  for do_asimut_file in ${do_asimut_VSTFILES[@]}
  do
    echo $do_asimut_file" C" >> "CATAL_ASIMUT_VASY"
  done
  do_asimut_CMD=$BIN"/asimut -bdd ""b"" ""b"" ""b""_simu"
  echo "["$do_asimut_CMD"]" >> "b.log"
  _TIME=`gdate +%s`
  EXEC=$do_asimut_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "b.log"
  echo $_TIME >> "b.log"
...
do_pat()
{
  ...
  do_pat_CMD=$BIN"/genpat ""b""_pat"
  echo "["$do_pat_CMD"]" >> "b.log"
  EXEC=$do_pat_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "b.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_vst()
{
  ...
  do_vst_CMD=$VHD2" -nomod "$SXLIB_COMP" ""b"".vhd"
  echo "["$do_vst_CMD"]" >> "b.log"
  EXEC=$do_vst_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "b.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_synth()
{
  ...
  echo "set output_file b.vhd" >> $do_synth_TCL
  ...
  echo "pd" >> $do_synth_TCL
  echo "optimize .work.MOD_b.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $do_synth_TCL
  echo "optimize_timing .work.MOD_b.main" >> $do_synth_TCL
  ...
  echo "auto_write -format VHD b.vhd" >> $do_synth_TCL
  echo "[Forking Leonardo Spectrum...]"
  do_synth_CMD=$SPECTRUM" -f "$do_synth_TCL
  echo "["$do_synth_CMD"]" >> "b.log"
  EXEC=$do_synth_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
  check
  cat "leospec.log" >> "b.log"
}
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    do_scram_DATE=$_DATE
    echo $do_scram_DATE >> "b.log"
    echo $do_scram_DATE
    echo "["$SCRAM" "$do_scram_SRC"]" >> "b.log"
...
do_patc()
{
  ...
  echo "#define N            300" >> $do_patc_PATC
  ...
  echo "  DEF_GENPAT(\"b\");" >> $do_patc_PATC
  do_patc_IND=1
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"d_0_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="d_0_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"d_1_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="d_1_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"d_2_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="d_2_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
  echo "  DECLAR(\"d_3_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $do_patc_PATC
  if [ "OUT" = "IN" ]
  then
    do_patc_PORTINP[$do_patc_IND]="d_3_rd"
  fi
  do_patc_IND=`expr $do_patc_IND + 1`
...
if [ $# = 0 ]
then
  echo "Usage: b.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
