...
do_scram()
{
  ...
  do
    ...
    echo $DATE >> "b.log"
    echo $DATE
    echo "["$SCRAM" "$SRC"]" >> "b.log"
    echo "["$SCRAM" "$SRC"]"
    _TIME=`gdate +%s`
    EXEC=$SCRAM" "$SRC
    $EXEC 2>&1 | tee -a "b.log"
    echo $_TIME >> "b.log"
    _EXEC_RES=$?
    res=$_EXEC_RES
    rm -rf "work._savant_lib"
...
do_synth()
{
  ...
  echo "set output_file b.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_b.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_b.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD b.vhd" >> $TCL
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" ""b"".vhd"
  echo "["$CMD"]" >> "b.log"
  EXEC=$CMD
  $EXEC 2>&1 | tee -a "b.log"
  _EXEC_RES=$?
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd ""b"" ""b"" ""b""_simu"
  echo "["$CMD"]" >> "b.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  $EXEC 2>&1 | tee -a "b.log"
...
if [ $# = 0 ]
then
  echo "Usage: tool.leonardo.b.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
