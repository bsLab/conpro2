#
# Tool Interface Tool leonardo.oar
# Version 2.05
#
TOP="oar"
OBJDIR="obj"
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd oar oar oar_simu"
  echo [$CMD"]" >> "oar.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "oar.log"
  echo $_TIME >> "oar.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" oar.vhd"
  echo [$CMD"]" >> "oar.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "oar.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file oar.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_oar.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_oar.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD oar.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"oar\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"array_xa_0_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="array_xa_0_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"array_xa_1_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="array_xa_1_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"top_d1\",\":2\",\"X\",INOUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "INOUT" = "IN" ]
  then
    PORTINP[$IND]="top_d1"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"top_d2\",\":2\",\"X\",INOUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "INOUT" = "IN" ]
  then
    PORTINP[$IND]="top_d2"
  fi
...
if [ $# = 0 ]
then
  echo "Usage: oar.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
