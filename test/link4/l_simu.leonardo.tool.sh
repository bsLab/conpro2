#
# Tool Interface Tool leonardo.l_simu
# Version 2.05
#
TOP="l_simu"
OBJDIR="obj"
...
VHDL[1]="l_simu"
VHDL[2]="/home/sbosse/proj/conpro2/test/link/out/l_connect"
VHDL[3]="/home/sbosse/proj/conpro2/test/link/out/l"
VHDL[4]="/home/sbosse/proj/conpro2/test/link/out/l_p1"
VHDL[5]="/home/sbosse/proj/conpro2/test/link/out/l_p2"
VHDL[6]="/home/sbosse/proj/conpro2/test/link/out/l_main"
do_xpat()
{
  echo [Starting xpat...]
  CMD="xpat -l obj/l_simu_simu.pat"
  EXEC=$CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  echo "l_simu C" >> "CATAL_ASIMUT_VASY"
  VSTFILES["1"]=""
  VSTFILES=( `ls "*.vst"` )
  ...
  CMD=$BIN"/asimut -bdd l_simu l_simu l_simu_simu"
  echo [$CMD"]" >> "l_simu.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l_simu.log"
  echo $_TIME >> "l_simu.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" l_simu.vhd"
  echo [$CMD"]" >> "l_simu.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l_simu.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file l_simu.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_l_simu.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_l_simu.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD l_simu.vhd" >> $TCL
...
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    DATE=$_DATE
    echo $DATE >> "l_simu.log"
    echo $DATE
    echo [$SCRAM" "$SRC"]" >> "l_simu.log"
    echo [$SCRAM" "$SRC"]"
    _TIME=`gdate +%s`
    EXEC=$SCRAM" "$SRC
    { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l_simu.log"
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"l_simu\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc2_x_rd\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc2_x_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc2_y_rd\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc2_y_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc2_xa_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc2_xa_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc2_ya_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc2_ya_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc1_x_rd\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc1_x_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc1_y_rd\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc1_y_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc1_xa_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc1_xa_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"lc1_ya_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="lc1_ya_rd"
  fi
  IND=`expr $IND + "1"`
  ...
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc1_DEV_ln_din\",\":2\",\"X\",OUT,\"19 downto 0\",\"\");" >> $PATC
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc2_DEV_ln_din\",\":2\",\"X\",OUT,\"19 downto 0\",\"\");" >> $PATC
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc1_DEV_ln_dout\",\":2\",\"X\",OUT,\"19 downto 0\",\"\");" >> $PATC
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc2_DEV_ln_dout\",\":2\",\"X\",OUT,\"19 downto 0\",\"\");" >> $PATC
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc1_DEV_ln_din_ack\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc2_DEV_ln_din_ack\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc1_DEV_ln_dout_ack\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"MON_Lc2_DEV_ln_dout_ack\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
...
init()
{
  ...
  cd "obj"
  rm -f "l_simu.log"; touch "l_simu.log"
}
check()
{
  if [ ! $res = "0" ]
...
if [ $# = 0 ]
then
  echo "Usage: l_simu.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
