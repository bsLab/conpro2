#
# Tool Interface Tool leonardo.l
# Version 2.06
#
TOP="l"
OBJDIR="obj"
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd l l l_simu"
  echo [$CMD"]" >> "l.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l.log"
  echo $_TIME >> "l.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" l.vhd"
  echo [$CMD"]" >> "l.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "l.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file l.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_l.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_l.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD l.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"l\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"x_rd\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="x_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"y_rd\",\":2\",\"X\",OUT,\"9 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="y_rd"
  fi
  ...
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_ln_din\",\":2\",\"X\",IN,\"1 downto 0\",\"\");" >> $PATC
  if [ "IN" = "IN" ]
  then
  ...
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_ln_dout\",\":2\",\"X\",OUT,\"1 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
  ...
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"ya_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="ya_rd"
  fi
...
if [ $# = 0 ]
then
  echo "Usage: l.leonardo.tool.sh [init vhdl synth simu xpat patc pat vst asimut]"
fi
for TARGET in $@ 
do
