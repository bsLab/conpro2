#
# Tool Interface Tool leonardo.com2
# Version 2.05
#
TOP="com2"
OBJDIR="obj"
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd com2 com2 com2_simu"
  echo [$CMD"]" >> "com2.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "com2.log"
  echo $_TIME >> "com2.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" com2.vhd"
  echo [$CMD"]" >> "com2.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "com2.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file com2.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_com2.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_com2.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD com2.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"com2\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data_in\",\":2\",\"B\",IN,\"\",\"\");" >> $PATC
  if [ "IN" = "IN" ]
  then
    PORTINP[$IND]="dev_data_in"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data_out\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="dev_data_out"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
...
if [ $# = 0 ]
then
  echo "Usage: com2.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
