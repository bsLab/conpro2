#
# Tool Interface Tool leonardo.com
# Version 2.05
#
TOP="com"
OBJDIR="obj"
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd com com com_simu"
  echo [$CMD"]" >> "com.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" com.vhd"
  echo [$CMD"]" >> "com.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "com.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file com.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_com.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_com.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD com.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"com\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data_in\",\":2\",\"X\",IN,\"31 downto 0\",\"\");" >> $PATC
  if [ "IN" = "IN" ]
  then
    PORTINP[$IND]="dev_data_in"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data_in_ack\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="dev_data_in_ack"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data_out\",\":2\",\"X\",OUT,\"31 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="dev_data_out"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data_out_ack\",\":2\",\"B\",IN,\"\",\"\");" >> $PATC
  if [ "IN" = "IN" ]
  then
    PORTINP[$IND]="dev_data_out_ack"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data\",\":2\",\"X\",OUT,\"15 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="dev_data"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"dev_data_en\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="dev_data_en"
  fi
  IND=`expr $IND + "1"`
...
if [ $# = 0 ]
then
  echo "Usage: com.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
