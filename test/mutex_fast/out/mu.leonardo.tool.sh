#
# Tool Interface Tool leonardo.mu
# Version 2.06
#
TOP="mu"
OBJDIR="obj"
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd mu mu mu_simu"
  echo [$CMD"]" >> "mu.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "mu.log"
  echo $_TIME >> "mu.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" mu.vhd"
  echo [$CMD"]" >> "mu.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "mu.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file mu.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_mu.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_mu.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD mu.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"mu\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d_0_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d_0_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d_1_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d_1_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d_2_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d_2_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d_3_rd\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d_3_rd"
  fi
...
if [ $# = 0 ]
then
  echo "Usage: mu.leonardo.tool.sh [init vhdl synth simu xpat patc pat vst asimut]"
fi
for TARGET in $@ 
do
