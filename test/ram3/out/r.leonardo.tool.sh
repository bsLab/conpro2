#
# Tool Interface Tool leonardo.r
# Version 2.06
#
TOP="r"
OBJDIR="obj"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" r.vhd"
  echo [$CMD"]" >> "r.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "r.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file r.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_r.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_r.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD r.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"r\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"mon_pro_p1_enable\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="mon_pro_p1_enable"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"mon_pro_p1_state\",\":2\",\"X\",OUT,\"2 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="mon_pro_p1_state"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"mon_pro_p2_enable\",\":2\",\"B\",OUT,\"\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="mon_pro_p2_enable"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"mon_pro_p2_state\",\":2\",\"X\",OUT,\"3 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="mon_pro_p2_state"
  fi
  IND=`expr $IND + "1"`
...
if [ $# = 0 ]
then
  echo "Usage: r.leonardo.tool.sh [init vhdl synth simu xpat patc pat vst asimut]"
fi
for TARGET in $@ 
do
