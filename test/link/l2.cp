...
type dev_type:{
  port ln_din: input logic[24];
  port ln_din_ack: output logic;
  port ln_dout: output logic[24];
...
object ln: link with datawidth=12;
...
begin
  ...
  --
  type l_connect: {
    port Lc1_ln_din: output logic[24];
    port Lc1_ln_din_ack: input logic;
    port Lc1_ln_dout: input logic[24];
    port Lc1_ln_dout_ack: output logic;
    port Lc2_ln_din: output logic[24];
    port Lc2_ln_din_ack: input logic;
    port Lc2_ln_dout: input logic[24];
