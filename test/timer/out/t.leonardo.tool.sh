...
{
  echo "[Starting xpat...]"
  do_xpat_CMD="xpat -l obj/t_simu.pat"
  EXEC=$do_xpat_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_asimut()
{
  ...
  do_asimut_CMD=$BIN"/asimut -bdd ""t"" ""t"" ""t""_simu"
  echo "["$do_asimut_CMD"]" >> "t.log"
  _TIME=`gdate +%s`
  EXEC=$do_asimut_CMD
...
do_pat()
{
  ...
  do_pat_CMD=$BIN"/genpat ""t""_pat"
  echo "["$do_pat_CMD"]" >> "t.log"
  EXEC=$do_pat_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "t.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_vst()
{
  ...
  do_vst_CMD=$VHD2" -nomod "$SXLIB_COMP" ""t"".vhd"
  echo "["$do_vst_CMD"]" >> "t.log"
  EXEC=$do_vst_CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "t.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
  rm -f /tmp/_EXEC_RES
...
do_synth()
{
  ...
  echo "set output_file t.vhd" >> $do_synth_TCL
  ...
  echo "pd" >> $do_synth_TCL
  echo "optimize .work.MOD_t.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $do_synth_TCL
  echo "optimize_timing .work.MOD_t.main" >> $do_synth_TCL
  ...
  echo "auto_write -format VHD t.vhd" >> $do_synth_TCL
  echo "[Forking Leonardo Spectrum...]"
  do_synth_CMD=$SPECTRUM" -f "$do_synth_TCL
  echo "["$do_synth_CMD"]" >> "t.log"
  EXEC=$do_synth_CMD
  $EXEC
  _EXEC_RES=$?
  res=$_EXEC_RES
...
do_scram()
{
  ...
  do
    ...
    _DATE=`date`
    do_scram_DATE=$_DATE
    echo $do_scram_DATE >> "t.log"
    echo $do_scram_DATE
    echo "["$SCRAM" "$do_scram_SRC"]" >> "t.log"
...
do_patc()
{
  ...
  echo "  DEF_GENPAT(\"t\");" >> $do_patc_PATC
  do_patc_IND=1
...
if [ $# = 0 ]
then
  echo "Usage: t.leonardo.tool.sh [init vhdl synth simu xpat patc]"
fi
for TARGET in $@ 
do
