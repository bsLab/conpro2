#
# Tool Interface Tool leonardo.q
# Version 2.06
#
TOP="q"
OBJDIR="obj"
...
do_asimut()
{
  ...
  CMD=$BIN"/asimut -bdd q q q_simu"
  echo [$CMD"]" >> "q.log"
  _TIME=`gdate +%s`
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "q.log"
  echo $_TIME >> "q.log"
...
do_vst()
{
  ...
  CMD=$VHD2" -nomod "$SXLIB_COMP" q.vhd"
  echo [$CMD"]" >> "q.log"
  EXEC=$CMD
  { $EXEC; echo $? > /tmp/_EXEC_RES; } 2>&1 | tee -a "q.log"
  _EXEC_RES=`cat /tmp/_EXEC_RES`
...
do_synth()
{
  ...
  echo "set output_file q.vhd" >> $TCL
  ...
  echo "pd" >> $TCL
  echo "optimize .work.MOD_q.main -target sxlib -macro -area -effort quick -hierarchy flatten" >> $TCL
  echo "optimize_timing .work.MOD_q.main" >> $TCL
  echo "report_area -cell_usage -hierarchy -all_leafs" >> $TCL
  echo "report_delay  -num_paths 1 -critical_paths -clock_frequency" >> $TCL
  echo "auto_write -format VHD q.vhd" >> $TCL
...
do_patc()
{
  ...
  echo "  int clk;" >> $PATC
  echo "  int i,j;" >> $PATC
  echo "  DEF_GENPAT(\"q\");" >> $PATC
  IND="1"
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d2_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d2_rd"
  fi
  IND=`expr $IND + "1"`
  echo "  DECLAR(\"d1_rd\",\":2\",\"X\",OUT,\"7 downto 0\",\"\");" >> $PATC
  if [ "OUT" = "IN" ]
  then
    PORTINP[$IND]="d1_rd"
  fi
...
if [ $# = 0 ]
then
  echo "Usage: q.leonardo.tool.sh [init vhdl synth simu xpat patc pat vst asimut]"
fi
for TARGET in $@ 
do
