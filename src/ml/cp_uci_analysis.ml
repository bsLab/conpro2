...
let syntax_of_file name =
  let file =  name in
  file_name := file;
  out (sprintf "Searching ucode file <%s>..." file);
  an.a_curpos <- nilsrc();
  let tops = ref [] in
  let lmap = ref [] in
  ...
  begin
    ...
    let prog = 
      ...
      do
        ...
        begin
            print_error (sprintf "Failed to read ucode file <%s>." file);
            exit 1;
        end;
        success := ok;
      done;
    ...
    *)
    ...
    do
        try
        begin
            let res = Cp_parser_uci.main Cp_lexer_uci.token lb in
            debug "syntax_of_file" with (sprintf "Parser returns (line %d)." 
                            (let l,p = line_of_pos {f_name= !file_name; f_cpos=lb.Lexing.lex_curr_pos} !lmap in l));
            tops := !tops @ [res];
        end
        with 
        | Cp_lexer_uci.EOF -> raise Exit;
        | _ ->
    ...
    debug "syntax_of_file" with (sprintf "#tops=%d #lmap=%d" (List.length !tops) (List.length !lmap));
    !tops,!lmap
  end
  with
