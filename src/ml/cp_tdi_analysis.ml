...
**    $INFO:
**
**  Tool description Interface - Analysis
**
**    $ENDOFINFO
**
*)
...
let syntax_of_file name =
  let file = sprintf "%s.tool" name in
  file_name := file;
  out (sprintf "Searching tool file <%s>..." file);
  an.a_curpos <- nilsrc();
  let tops = ref [] in
  let lmap = ref [] in
  ...
  begin
    ...
    let prog = 
      ...
      do
        let ok = protects (
            ...
            out (sprintf "Opening tool file <%s>..." file);
        ...
        begin
            print_error (sprintf "Failed to read tool file <%s>." file);
            exit 1;
        end;
        success := ok;
      done;
    ...
    *)
    ...
    do
        try
        begin
            let res = Cp_parser_tdi.main Cp_lexer_tdi.token lb in
            tops := !tops @ [res];
        end
        with 
        | Cp_lexer_tdi.EOF -> raise Exit;
        | _ ->
