...
let mutable c_et= []
let mutable c_exc_map= false
let mutable c_defs= [] 
let mutable c_evals= [] 
let mutable c_exc_env= 0 
let mutable c_mod_dep= []
let mutable c_expr_close= ""
...
let c_emit_types env  =
  ...
  List.iter (fun obj ->
    match obj with
    | C_struct st ->
      let mutable s= (sprintf "struct %s {\n" (fix_name st.strut_name)) in
      List.iter2 (fun (el,dt) el' ->
        s <- sprintf "%s  %s %s;\n" s 
                    (match dt with
      ...
      out s;
    | C_enum en ->
      let mutable s= (sprintf "typedef enum { \n" ) in
      let mutable n= (List.length en.enum_elems) in
      let mutable i= 0 in
      List.iter (fun el ->
        n <- n-1; i <- i+1;
...
let rec c_op et op op1 op2 =
  ...
  | DT_logic n | DT_int n ->
    ...
    begin
      ...
      | OP_max -> sprintf  "MAX64(%s,%s)" op1 op2
      | OP_min -> sprintf  "MIN64(%s,%s)" op1 op2
      | OP_eq  -> sprintf  "%s == %s" op1 op2
      | OP_neq -> sprintf  "%s != %s" op1 op2
      | OP_lt  -> sprintf  "%s < %s" op1 op2
      | OP_le  -> sprintf  "%s <= %s" op1 op2
          ...
          begin
            ...
            sprintf "[%s]" (pargs 
                              (List.map2 (fun d e -> 
                                ...
                                if first' then (c_expr env  false (DT_int 0) e)
                                else sprintf "%s*%d" (c_expr env  false (DT_int 0) e) last' ) (List.rev dims) (List.rev sels)));
          end;
          | _ -> 
...
and c_expr env  lhs et pi =
  ...
  | PI_concat (op1,op2) -> 
    ...
    let rec iter pi =
      ...
      | PI_concat (op1',op2') ->
        nl <- (typ op1') :: nl;
        (c_expr env false et op1') :: (iter op2')
      | _ -> nl <- nl @ [typ pi];
            [c_expr env false et pi]  in
    let opl = (c_expr env false et op1) :: (iter op2) in
    let mutable s = "" in
    let mutable n = 0 in
    List.iter2 (fun opn dt ->
      if n > 0 then s <- sprintf "(%s << %d)|%s"  opn n s
...
let c_emit_config () =
  (*
  ** Read template conpro_config.h...
  *)
  let fname = "conpro_config.h" in
  let pathl = ""::(compiler.t_lib@compiler.t_incl) in        
  let mutable code= [] in
  let rec f_open pathl =
    match pathl with
...
let c_emit modu =
  ...
  *)
  List.iter (fun pro ->
    ...
    begin
      ...
      out (sprintf "#include \"%s_config.h\"" (of_mod modu.mod_name));
  ...
  *)
  ...
  out "event_t *conpro_exit;";  
  out "int conpro(){";
  ind_incr ();
  out "process_init();";
  out "conpro_exit=event_new(\"\");";
  out "conpro_exit->init(conpro_exit);";
  List.iter (fun init ->
    out init;
    ) main_init;
  (*
  ...
  *)
  ...
  out "extern event_t *conpro_exit;";  
  finish_int fname;
  ...
  *)
  ...
  let fname = sprintf "%s_config" (of_mod modu.mod_name) in
  create_int fname;  
  c_emit_config ();
  finish_int fname;
