...
**    $INFO:
**
**  Tools defintion File (tools.def) - Analysis
**
**    $ENDOFINFO
**
*)
...
let syntax_of_file name =
  let file = sprintf "%s.def" name in
  file_name := file;
  out (sprintf "Searching tools file <%s>..." file);
  an.a_curpos <- nilsrc();
  let tops = ref [] in
  let lmap = ref [] in
  ...
  begin
    ...
    let prog = 
      ...
      do
        let ok = protects (
            ...
            out (sprintf "Opening tools file <%s>..." file);
        ...
        begin
            print_error (sprintf "Failed to read tools file <%s>." file);
            exit 1;
        end;
    ...
    *)
    ...
    do
        try
        begin
            let res = Cp_parser_tools.main Cp_lexer_tools.token lb in
            tops := !tops @ [res];
        end
        with 
        | Cp_lexer_tools.EOF -> raise Exit;
        | _ ->
            let pos = lb.Lexing.lex_curr_pos in
            let l,p = line_of_pos {f_name= !file_name; f_cpos=pos} !lmap in
            out (sprintf "#tops=%d" (List.length !tops));
