...
let usage str =
    let authorized = true in (* license_check "CONPRO2" in *)
...
let main () =
  ...
  begin
    ...
    let rec iter args =
      try
        ...
        begin
            ...
            | "-notty" -> compiler.t_notty <- true; iter tl;
            | "-parse" -> compiler.t_parse <- true; iter tl;
            | "+report" -> compiler.t_report <- ["report"]; iter tl;
            ...
            begin
              ...
              | [feature;hostid;file] -> 
                protect(license_gen feature hostid file false); exit 0;
    ...
    begin
      try __(Unix.opendir compiler.t_output) with _ -> 
    ...
    begin  
      ...
      begin
        List.iter (fun cp -> 
          ...
          *)
          out_ (sprintf "Reading & Analyzing ConPro source <%s>..." cp);
